...
function getEscrowKeeperSigner(): Account {
  ...
  if (privateKey !== undefined && privateKey.length > 0) {
    ...
    // Shelby upload signer, so the keeper shares the configured on-chain identity.
  ...
  if (process.env.NODE_ENV === 'production') {
    console.warn(
      '[EscrowKeeper] SHELBY_SIGNER_PRIVATE_KEY is not set. auto_release txs will use an ' +
      'ephemeral, likely unfunded account and will fail for gas.',
    );
  }
